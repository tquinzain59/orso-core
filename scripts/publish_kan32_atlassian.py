"""Publication de la spécification KAN-32 sur Confluence et création / clôture du ticket KAN-32 dans Jira."""
...
# 1. Publication Confluence KAN-32
spec_path = "docs/3_Technique/spec_kan32_canaux_messaging_backoffice.md"
with open(spec_path, "r", encoding="utf-8") as f:
    spec_md = f.read()

storage_html = markdown_to_confluence_storage(spec_md)
page_title = "KAN-32 - Spec : Raccordement Dynamique des Canaux de Discussion au Backoffice Hermès"

check_url = f"https://{DOMAIN}.atlassian.net/wiki/rest/api/content?spaceKey=Orsoagents&title={urllib.parse.quote(page_title)}&expand=version"
...
# 2. Gestion du ticket Jira KAN-32
...
def create_or_get_issue(summary: str, description_text: str):
    search_url = f"https://{DOMAIN}.atlassian.net/rest/api/3/search/jql?jql={urllib.parse.quote('project = KAN AND summary ~ \"Canaux\"')}"
...
# Création et Clôture KAN-32
kan32_desc = (
    "Raccordement dynamique des passerelles de messagerie de l'onglet 'Canaux' (apps/ui-client) au moteur Hermès. "
    "Suppression définitive des numéros, emails et compteurs de messages factices au profit d'un sondage d'environnement réel "
    "(TELEGRAM_BOT_TOKEN, WHATSAPP_TOKEN, SMTP_HOST, SLACK_BOT_TOKEN, DISCORD_BOT_TOKEN). "
    "Statuts transparents (Connecté, Non configuré), gestion des utilisateurs autorisés et modale de paramétrage avec instructions pas-à-pas."
)
kan32_key = create_or_get_issue(
    "UI Client & Backend : Raccordement dynamique des Canaux de Discussion (WhatsApp, Telegram, Email) au Backoffice Hermès",
    kan32_desc,
)
kan32_comment = (
    f"Jalon validé, testé et déployé en production le 25/09/2026 sur https://prod-fr-002.orso-agents.fr.\n"
    f"- Élimination complète des données factices (faux numéros +336..., faux compteurs de messages).\n"
    f"- Implémentation du routeur _probe_hermes_backoffice_channels dans hermes_cli/web_routers/client_ui.py.\n"
    f"- Détection en temps réel de TELEGRAM_BOT_TOKEN, WHATSAPP_TOKEN, SMTP_HOST, SLACK_BOT_TOKEN, DISCORD_BOT_TOKEN.\n"
    f"- Modale d'instructions détaillées (@BotFather, SMTP, WhatsApp Cloud) et gestion des accès autorisés.\n"
    f"- 19 tests unitaires validés (100% green via tests/hermes_cli/test_client_ui.py).\n"
    f"- Recompilation du bundle PWA (npm run build).\n"
    f"Documentation Confluence associée : {confluence_page_url}"
)
transition_and_comment(kan32_key, "51", kan32_comment)

print("Synchronisation Atlassian KAN-32 terminée avec succès !")
